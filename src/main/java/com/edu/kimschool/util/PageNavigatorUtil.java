...
public class PageNavigatorUtil {
	 ...
	 * 생성자
	 */
	public PageNavigatorUtil(int countPerPage, int pagePerGroup, int currentPage, int totalRecordsCount) {
		...
		// 전체 페이지 수
		totalPageCount = (totalRecordsCount + countPerPage - ConstEnum.Page.FIRST_PAGE.getValue()) / countPerPage;

		// 전달된 현재 페이지가 1보다 작으면 현재페이지를 1페이지로 지정
		if (currentPage < ConstEnum.Page.FIRST_PAGE.getValue())
			currentPage = ConstEnum.Page.FIRST_PAGE.getValue();
		...
		// 현재 그룹
		currentGroup = (currentPage - ConstEnum.Page.FIRST_PAGE.getValue()) / pagePerGroup;

		// 현재 그룹의 첫 페이지
		startPageGroup = currentGroup * pagePerGroup + ConstEnum.Page.FIRST_PAGE.getValue();
		// 현재 그룹의 첫 페이지가 1보다 작으면 1로 처리
		// ?(삼항연산자) : ?에 대한 좌항의 조건이 참이면 :의 왼쪽의 처리, 거짓이면 :의 오른쪽을 처리
		startPageGroup = startPageGroup < ConstEnum.Page.FIRST_PAGE.getValue()
				? ConstEnum.Page.FIRST_PAGE.getValue()
				: startPageGroup;
		// 현재 그룹의 마지막 페이지
		endPageGroup = startPageGroup + pagePerGroup - ConstEnum.Page.FIRST_PAGE.getValue();
		...
		startRecord = (currentPage - ConstEnum.Page.FIRST_PAGE.getValue()) * countPerPage;
	}
	...
	// @Override : String클래스에 있는 toString메소드를 사용하는데, 다시 재정의를 한다.
	@Override
